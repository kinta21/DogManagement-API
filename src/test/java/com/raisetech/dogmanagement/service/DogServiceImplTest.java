...
class DogServiceImplTest {
    ...
    class UpdateDogTest{
        ...
        public void 指定したIDのデータ情報を入力データで更新ができること() {
            doReturn(Optional.of(new Dog(1, "シロ", "オス", "1歳", "フレンチブルドック", "東北"))).when(dogMapper).findById(1);
            dogServiceImpl.updateDog(1, "おはぎ", "オス", "1歳", "パグ", "東北");

            verify(dogMapper, times(1)).findById(1);
            verify(dogMapper, times(1)).updateDog(new Dog(1, "おはぎ", "オス", "1歳", "パグ", "東北"));
        }
        ...
        public void 更新指定した犬のIDが存在しないとき例外を返すこと() {
            ...
            assertThatThrownBy(() -> dogServiceImpl.updateDog(99, "おはぎ", "オス", "1歳", "パグ", "東北"))
